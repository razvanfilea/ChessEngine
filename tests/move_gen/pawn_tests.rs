...
use lucky_chess::move_gen::{generate_pawn_moves, AllMoves, Black, GenCaptures, GenQuiets, MoveGenType, MoveList, Player, White};
use chess_base::prelude::*;

fn get_moves<Us: Player, Type: MoveGenType>(board: &Board) -> Vec<(Sq, Sq, MoveFlags)> {
    let mut moves = MoveList::default();
    let new_pos = generate_pawn_moves::<Us, Type>(board, moves.as_ptr());
...
fn test_white_pawn_pushes() {
    ...
    let moves = get_moves::<White, AllMoves>(&board);
...
fn test_black_pawn_pushes() {
    ...
    let moves = get_moves::<Black, AllMoves>(&board);
    assert_eq!(moves.len(), 2);
...
fn test_gen_captures_only() {
    let board = Board::from_fen("8/8/8/8/8/1p1p4/2P5/8 w - - 0 1").unwrap();
    let moves = get_moves::<White, GenCaptures>(&board);
...
fn test_gen_quiets_only() {
    let board = Board::from_fen("8/8/8/8/8/1p1p4/2P5/8 w - - 0 1").unwrap();
    let moves = get_moves::<White, GenQuiets>(&board);
