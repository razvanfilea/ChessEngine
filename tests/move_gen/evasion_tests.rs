...
use lucky_chess::move_gen::{generate_moves, Black, Evasions, White};
...
fn test_double_check_only_king_moves() {
    ...
    for m in moves.as_slice() {
        assert_eq!(m.from(), Sq::E4, "Only King moves are valid in double check");
    }
}
...
fn test_single_check_interposition_blocking() {
    ...
    let knight_moves: Vec<_> = moves.as_slice().iter().filter(|m| m.from() == Sq::C3).collect();
    ...
    let king_moves: Vec<_> = moves.as_slice().iter().filter(|m| m.from() == Sq::E1).collect();
...
fn test_single_check_capture_of_checker() {
    ...
    let pawn_c2_cap = moves.as_slice().iter().any(|m| m.from() == Sq::C2 && m.to() == Sq::D3 && m.is_capture());
    let pawn_e2_cap = moves.as_slice().iter().any(|m| m.from() == Sq::E2 && m.to() == Sq::D3 && m.is_capture());

    assert!(pawn_c2_cap, "Pawn on c2 should be able to capture checker on d3");
    assert!(pawn_e2_cap, "Pawn on e2 should be able to capture checker on d3");
}
...
fn test_black_evasions_single_check() {
    ...
    let moves = generate_moves::<Black, Evasions>(&board);

    let knight_blocks: Vec<_> = moves.as_slice().iter().filter(|m| m.from() == Sq::C6).collect();
    assert_eq!(knight_blocks.len(), 2);
    assert!(knight_blocks.iter().any(|m| m.to() == Sq::E7));
    assert!(knight_blocks.iter().any(|m| m.to() == Sq::E5));

    let king_moves: Vec<_> = moves.as_slice().iter().filter(|m| m.from() == Sq::E8).collect();
    assert!(!king_moves.is_empty());
}
